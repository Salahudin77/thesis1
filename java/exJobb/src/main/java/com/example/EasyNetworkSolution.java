...
public class EasyNetworkSolution {

    public static void main(String[] args) throws Exception {
        // First, measure model creation time
        System.out.println("==== Creating network model ====");
        long startCreateTime = System.nanoTime();
        MultiLayerNetwork model = createEasyNetwork();
        long endCreateTime = System.nanoTime();
        double createTimeMs = (endCreateTime - startCreateTime) / 1e6;
        System.out.printf("Model creation time: %.2f ms\n", createTimeMs);

        // Next, measure training time
        System.out.println("\n==== Training network model ====");
        TrainingResult trainingResult = trainNetwork(model);
        System.out.printf("Training time: %.2f ms\n", trainingResult.timeMs);
        ...
        System.out.println("Warm-up complete, starting inference benchmark...");
        ...
        for (int i = 0; i < runs; i++) {
            System.out.printf("\n--- Inference Test Run %d ---\n", i + 1);
            InferenceResult result = testNetwork(model, imageFile);
            inferenceResults.add(result);
        }

        // Aggregate inference metrics
        double avgInferenceTime = average(inferenceResults.stream().mapToDouble(r -> r.timeMs).toArray());
        double stdDevInferenceTime = stdDev(inferenceResults.stream().mapToDouble(r -> r.timeMs).toArray());

        double avgInferenceMemory = average(inferenceResults.stream().mapToDouble(r -> r.memoryUsedMb).toArray());
        double stdDevInferenceMemory = stdDev(inferenceResults.stream().mapToDouble(r -> r.memoryUsedMb).toArray());

        double avgInferencePeakMemory = average(inferenceResults.stream().mapToDouble(r -> r.peakMemoryMb).toArray());
        double stdDevInferencePeakMemory = stdDev(inferenceResults.stream().mapToDouble(r -> r.peakMemoryMb).toArray());
        ...
        System.out.printf("Average inference time: %.2f ms (±%.2f)\n", avgInferenceTime, stdDevInferenceTime);
        System.out.printf("Average inference memory used: %.2f MB (±%.2f)\n", avgInferenceMemory, stdDevInferenceMemory);
        System.out.printf("Average inference peak memory: %.2f MB (±%.2f)\n", avgInferencePeakMemory, stdDevInferencePeakMemory);
        ...
        System.out.printf("Training peak memory: %.2f MB\n", trainingResult.peakMemoryMb);
        System.out.printf("Average inference time: %.2f ms (±%.2f)\n", avgInferenceTime, stdDevInferenceTime);
        System.out.printf("Total time (creation + training + avg inference): %.2f ms\n",
                createTimeMs + trainingResult.timeMs + avgInferenceTime);
        ...
        saveToJson(
                ...
                stdDevInferenceTime,
                avgInferenceMemory,
                stdDevInferenceMemory,
                avgInferencePeakMemory,
                stdDevInferencePeakMemory,
                commonPrediction
        );

        ModelSerializer.writeModel(model, "trained-easy-model.zip", true);
    }
    ...
    // Separate result classes for inference and training
...
static void saveToJson(
        ...
        double stdDevInferenceTime,
        double avgMemory,
        double stdDevMemory,
        double avgPeakMemory,
        double stdDevPeakMemory,
        int commonPrediction) throws Exception {
    ...
    // Add model creation info
    root.put("model_creation", new JSONObject()
            .put("time_ms", createTimeMs));

    // Add training info - exclude timeseries
    JSONObject trainingObject = new JSONObject();
    trainingObject.put("time_ms", trainingResult.timeMs);
    ...
    trainingObject.put("accuracy", trainingResult.accuracy);
    trainingObject.put("memory_profile", trainingResult.memoryProfile.getJSONObject("summary")); // Only include summary
    ...
    for (int i = 0; i < inferenceResults.size(); i++) {
        ...
        runObj.put("memory_profile", r.memoryProfile.getJSONObject("summary")); // Only include summary
    ...
    root.put("summary", new JSONObject()
            .put("model_creation_time_ms", createTimeMs)
            .put("training_time_ms", trainingResult.timeMs)
            .put("training_peak_memory_mb", trainingResult.peakMemoryMb)
            .put("average_inference_time_ms", avgInferenceTime)
            .put("std_dev_inference_time_ms", stdDevInferenceTime)
            .put("average_inference_memory_mb", avgMemory)
            .put("std_dev_inference_memory_mb", stdDevMemory)
            .put("average_inference_peak_memory_mb", avgPeakMemory)
            .put("std_dev_inference_peak_memory_mb", stdDevPeakMemory)
            .put("most_common_prediction", commonPrediction)
            .put("total_time_ms", createTimeMs + trainingResult.timeMs + avgInferenceTime));

    try (FileWriter file = new FileWriter("easy_memory_results256.json")) {
        file.write(root.toString(4)); // pretty print
        System.out.println("Results exported to easy_memory_results.json");
    }
}
    ...
    static TrainingResult trainNetwork(MultiLayerNetwork model) throws Exception {
        System.gc();
        Thread.sleep(100);
        ...
        try (MemoryTracker memoryTracker = new MemoryTracker(100)) { // Use 100ms intervals for training since it's longer
            memoryTracker.start();
            long startTime = System.nanoTime();
            ...
            // Create memory profile JSON
            // Create memory profile JSON - only include summary
            JSONObject memoryProfile = new JSONObject();
            memoryProfile.put("summary", memoryTracker.getSummary());
            // Don't include timeseries data
            return new TrainingResult(timeMs, memoryUsedMb, cpuTimeMs, peakMemoryMb, accuracy, memoryProfile);
        }
    }

    static MultiLayerNetwork createEasyNetwork() {
